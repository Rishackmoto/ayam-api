...
// GET semua jenis sales
router.get('/', async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query('SELECT * FROM jenissales');
...
router.put('/:kode', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      ...
      .query('UPDATE jenissales SET jenis = @jenis WHERE kode = @kode');
...
router.delete('/:kode', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      .input('kode', sql.VarChar, kode)
      .query('DELETE FROM jenissales WHERE kode = @kode');
