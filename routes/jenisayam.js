...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await pool.request().query('SELECT * FROM JenisAyam');
...
router.put('/:kode', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      ...
      .query('UPDATE JenisAyam SET jenis = @jenis WHERE kode = @kode');
...
router.delete('/:kode', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      .input('kode', sql.VarChar, kode)
      .query('DELETE FROM JenisAyam WHERE kode = @kode');
