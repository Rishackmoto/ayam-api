...
// GET semua jenis satuan
router.get('/', async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query('SELECT * FROM jenissatuan');
...
router.put('/:kode', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      ...
      .query('UPDATE jenissatuan SET jenis = @jenis WHERE kode = @kode');
...
router.delete('/:kode', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      .input('kode', sql.VarChar, kode)
      .query('DELETE FROM jenissatuan WHERE kode = @kode');
